...
List Nmix_mcmc(IntegerMatrix y, NumericVector x, NumericMatrix w, int nmcmc) {
  ...
  for (int k=0; k<nmcmc; ++k){
    ...
    for (int i=0; i<nsite; ++i) {
      ...
      double logit_lam = beta[0] + beta[1] * x(i);
      double lam = R::plogis(logit_lam, 0, 1, true, false);
      double lamb = kappa * lam;

      double mh1N = 0; 
      double mh2N = 0; 
      mh1N += R::dpois(N[i], N_star[i] + N_tune, true) + R::dpois(N_star[i], lamb, true);
      mh2N += R::dpois(N_star[i], N[i] + N_tune, true) + R::dpois(N[i], lamb, true);
    ...
    for (int i=0; i<nsite; ++i) {
      double logit_lam_star = beta_star[0] + beta_star[1] * x(i);
      double logit_lam = beta[0] + beta[1] * x(i);
      double lam_star = R::plogis(logit_lam_star, 0, 1, true, false);
      double lam = R::plogis(logit_lam, 0, 1, true, false);
      double lamb_star = kappa_star * lam_star;
      double lamb = kappa * lam;
      mh1B += R::dpois( N(i), lamb_star, true );
      mh2B += R::dpois( N(i), lamb, true );
    } // i
    ...
    for (int i=0; i<nsite; ++i) {
      for (int j=0; j<nreps; ++j) {
        double logit_p_star = alpha_star[0] + alpha_star[1] * w(i,j);
        double logit_p = alpha[0] + alpha[1] * w(i,j);
